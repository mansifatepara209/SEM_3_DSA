        ...
        while (x < 6) {
            ...
            switch (x) {
                case 1:
                    ...
                    if (root == null)
                        root = n; // Set the new node as root if tree is empty
                    else
                        pb.insertMobileNumber(n, root); // Insert the new node into the BST
                    break;
...
class PhoneBook {
    ...
    public Node delete(Node root, String name) {
        ...
        if (root.name.compareTo(name) < 0)
            root.right = delete(root.right, name); // Recur on right subtree
        if (root.name.compareTo(name) > 0)
            root.left = delete(root.left, name); // Recur on left subtree
